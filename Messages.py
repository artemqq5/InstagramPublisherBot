START_MESSAGE = "Hello, use /help to get all commands"

HELP_MESSAGE = "\n" \
               "/set_token - sets your personal token to communicate with instagram account\n\n" \
               "/get_all_posts - get all post for all time\n\n" \
               "/publish_post - publication media files\n\n" \
               "/account - get info about account\n"

YOU_NEED_REGISTER = "You are not register, use /start to register"
CHECK_ACCESS_TOKEN = "Error. Check your access-token and try again"
SUCCESSFUL_GENERATE_TOKEN = "Your 60-day token has been successfully generated. You need to renew the token," \
                            " if you don`t used it for 60 days, each request will renew its expiration again"

GET_MEDIA_EXCEPTION = "An error occurred while retrieving media files, please try again or refresh the token"

EXCEPTION_AUTHORISATION = "You are not authorised or token was deprecated. /start or /set_token"

EXCEPT_CREATE_POST = "Exception when create post"
